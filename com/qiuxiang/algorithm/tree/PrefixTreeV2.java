 ...
 */
public class PrefixTreeV2 {
    public TrieNode root;

    public PrefixTreeV2(TrieNode root) {
        this.root = root;
    }
    ...
    public static class TrieNode {
        public int passCount; // 经过该节点的字符串数量
        public int endCount; // 以该节点结尾的字符串数量
        public HashMap<Integer, TrieNode> next;

        public TrieNode() {
            this.passCount = 0;
            this.endCount = 0;
            this.next = new HashMap<>(); // 使用HashMap来存储子节点
        }
    ...
    public void insert(String word) {
        ...
        for (char c : charArray) {
            int index = c - 'a';
            node.next.computeIfPresent(index, (k, v) -> {
                v.passCount++;
                return v;
            });
    ...
    public int search(String word, boolean isPrefix) {
        ...
        for (char c : charArray) {
            int index = c - 'a';
            TrieNode trieNode = node.next.get(index);
            if (trieNode == null) {
                return 0;
            }
            node = trieNode;
    ...
    public void delete(String word) {
        ...
        for (char c : charArray) {
            int index = c - 'a';
            if (node.next.get(index).passCount == 1) {
                node.next.remove(index);
            } else {
                node.next.get(index).passCount--;
